...
export default function ReportsPage() {
  ...
  return <div className="page-stack">
    ...
    <div className="button-row"><button className={tab === 'occupancy' ? '' : 'secondary'} onClick={() => setTab('occupancy')}>Occupancy</button><button className={tab === 'revenue' ? '' : 'secondary'} onClick={() => setTab('revenue')}>Revenue/Payment</button><button className={tab === 'arrival' ? '' : 'secondary'} onClick={() => setTab('arrival')}>Arrival/Departure</button><button className={tab === 'room' ? '' : 'secondary'} onClick={() => setTab('room')}>Room Status</button></div>
    ...
    {!loading && report && tab === 'revenue' && <div className="grid"><div className="card"><h3>Invoice total</h3><p>{money.format(report.revenue.invoice_total)}</p></div><div className="card"><h3>Payment collected</h3><p>{money.format(report.revenue.payment_collected)}</p></div><div className="card"><h3>Outstanding balance</h3><p>{money.format(report.revenue.outstanding_balance)}</p></div></div>}

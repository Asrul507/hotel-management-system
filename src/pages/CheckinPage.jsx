  ...
  return <div className="page-stack">
    ...
    <div className="two-column">
      ...
      <div className="card table-card"><h2>Tamu In-house</h2>{loading ? <p>Memuat stay...</p> : activeStays.length === 0 ? <p className="muted">Belum ada tamu in-house.</p> : <table><thead><tr><th>Tamu</th><th>Kamar</th><th>Check-in</th><th>Expected Out</th><th>Billing</th><th>Aksi</th></tr></thead><tbody>{activeStays.map((stay) => {
        ...
        return <tr key={stay.id}><td>{stay.guests?.full_name}</td><td>{stay.rooms?.room_number}<br /><small>{stay.rooms?.hk_status}</small></td><td>{stay.checkin_at?.slice(0, 16).replace('T', ' ')}</td><td>{stay.reservations?.check_out_date || '-'}</td><td><span className={`badge ${billingStatus}`}>{folio ? getBillingStatusLabel(folio) : billingStatus.toUpperCase()}</span><br /><small>{folio?.folio_number ? `${folio.folio_number} - ` : ''}{money.format(balanceDue)} due</small></td><td><div className="table-actions compact-actions"><IconButton icon={faRightFromBracket} title="Check-out" disabled={processing === stay.id} variant="primary" onClick={() => run(stay.id, () => staysApi.checkOut(stay), `Check-out ${stay.guests?.full_name}? Folio akan ditutup jika belum closed.`)} />{canMoveRoom && <IconButton icon={faRightLeft} title="Pindah Kamar" disabled={processing === `move-load-${stay.id}`} onClick={() => openMoveRoom(stay)} />}</div></td></tr>;
      })}</tbody></table>}</div>
    </div>
  </div>;
}

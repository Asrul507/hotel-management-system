  ...
  async function action(reservation, status) {
    ...
    if (status === 'cancelled') {
      const result = await dialog.form({
        ...
        message: `Lengkapi informasi cancel untuk reservasi ${reservation.reservation_code || '-'}.`,
        confirmLabel: 'Cancel Reservasi',
        danger: true,
        fields: [
          { name: 'cancellation_reason', label: 'Alasan cancel', defaultValue: '', autoFocus: true, full: true },
          { name: 'cancellation_fee', label: 'Cancellation fee', type: 'number', min: '0', defaultValue: '0' },
          { name: 'refund_amount', label: 'Refund deposit', type: 'number', min: '0', defaultValue: '0' }
        ]
      });
      if (!result.confirmed) return;
      options.cancellation_reason = result.values.cancellation_reason || '';
      options.cancellation_fee = Number(result.values.cancellation_fee || 0);
      options.refund_amount = Number(result.values.refund_amount || 0);
    }
    if (status === 'no_show') {
      const result = await dialog.form({ title: 'No-show Reservasi', message: `Tandai reservasi ${reservation.reservation_code || '-'} sebagai no-show?`, confirmLabel: 'Mark No-show', danger: true, fields: [{ name: 'no_show_fee', label: 'No-show fee', type: 'number', min: '0', defaultValue: '0', autoFocus: true }] });
      if (!result.confirmed) return;
      options.no_show_fee = Number(result.values.no_show_fee || 0);
    }
    if (status !== 'cancelled' && status !== 'no_show') {
      const confirmed = await dialog.confirm({ title: 'Update Reservasi', message: `Konfirmasi ${status} reservasi ${reservation.reservation_code}?`, confirmLabel: 'Konfirmasi' });
      if (!confirmed) return;
    }

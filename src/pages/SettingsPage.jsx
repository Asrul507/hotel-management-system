...
export default function SettingsPage() {
  ...
  return <div className="page-stack">
    ...
    <div className="two-column">
      ...
      <div className="card detail-list"><h2>Hotel</h2>{loading ? <p>Memuat setting...</p> : <><p><strong>Nama hotel:</strong> {hotel?.hotel_name || '-'}</p><p><strong>Alamat:</strong> {hotel?.address || '-'}</p><p><strong>Telepon:</strong> {hotel?.phone || '-'}</p><p><strong>Pajak:</strong> {hotel?.tax_percent ?? 0}%</p><p><strong>Service charge:</strong> {hotel?.service_charge_percent ?? 0}%</p><p><strong>Prefix invoice:</strong> {hotel?.invoice_prefix || 'INV'}</p></>}</div>
    </div>
  </div>;
}
